...
class ExerciseService {
  static final List<Exercise> _exercises = [
    ...
    const Exercise(
      ...
      type: ExerciseType.push,
      description: 'A variation of the push-up that places more emphasis on the lower chest. Easier than a standard push-up.',
      difficulty: 1,
    ),
    const Exercise(
      id: 'dips',
      name: 'Dips',
      type: ExerciseType.push,
      description: 'A compound exercise that targets the triceps, chest, and shoulders. Requires parallel bars or a sturdy edge.',
    ...
    const Exercise(
      ...
      name: 'Pike Push-ups',
      type: ExerciseType.push,
      description: 'A challenging push-up variation that shifts the focus to the shoulders.',
      difficulty: 3,
    ),
    const Exercise(
      id: 'db_bench_press',
      ...
      type: ExerciseType.push,
      description: 'A fundamental upper body exercise using dumbbells to work the chest, triceps, and shoulders.',
    ...
    const Exercise(
      ...
      description: 'A challenging bodyweight exercise that targets the back and biceps. Requires a pull-up bar.',
    ...
    const Exercise(
      ...
      name: 'Inverted Rows',
      type: ExerciseType.pull,
      description: 'A bodyweight exercise that targets the back and biceps. Can be done with a bar or a sturdy table.',
      difficulty: 2,
    ),
     const Exercise(
      id: 'db_row',
    ...
    const Exercise(
      ...
      name: 'Lunges',
      type: ExerciseType.legs,
      description: 'A unilateral leg exercise that works the quads, glutes, and hamstrings.',
      difficulty: 2,
    ),
    const Exercise(
      id: 'step_up',
      name: 'Step-ups',
      type: ExerciseType.legs,
      description: 'An exercise that involves stepping up onto a raised platform, targeting the quads and glutes.',
      difficulty: 1,
    ),
    const Exercise(
      id: 'glute_bridge',
    ...
    const Exercise(
      ...
      name: 'Goblet Squat',
      type: ExerciseType.legs,
      description: 'A squat variation holding a single dumbbell or kettlebell at chest height.',
      requiredEquipment: [Equipment.dumbbells],
      difficulty: 3,
    ),
    ...
    const Exercise(
      ...
      name: 'Shadow Boxing',
      type: ExerciseType.conditioning,
      description: 'A combat sport exercise in which a person throws punches at the air as if they were fighting an opponent.',
      difficulty: 2,
    ),
  ];
...
List<Exercise> getExercisesForFocus(List<FocusArea> focusAreas, {required Goal goal, List<Equipment> availableEquipment = const [Equipment.none]}) {
  ...
  final wantConditioning = goal == Goal.loseWeight || goal == Goal.stayInShape;
  ...
  final filtered = _exercises.where((e) {
    ...
    if (e.requiredEquipment.isNotEmpty) {
      final has = e.requiredEquipment.any((req) => availableEquipment.contains(req) || availableEquipment.contains(Equipment.fullGym));
      if (!has) return false;
    }
  ...
  if (wantConditioning) {
    final conditioning = filtered.where((e) => e.type == ExerciseType.conditioning).toList();
